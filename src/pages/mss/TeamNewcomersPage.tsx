...
import { UserCheck, Check, Play, Circle, Zap, Target, AlertTriangle, ShieldAlert } from 'lucide-react';
  ...
  return (
    <div className="animate-fade-up space-y-5">
        ...
        return (
          <Card key={n.emp.id}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <Link to="/team/equipe"><Button variant="ghost" size="sm">Vue 360°</Button></Link>
            </div>

            {probaIn <= 14 && (
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-warn/[0.08] px-3 py-2.5">
                <p className="flex items-center gap-1.5 text-[12px] font-semibold text-warn"><AlertTriangle size={13} /> Décision fin de période d’essai à prendre avant le {frDate(n.probationEnd)} (J-{probaIn}).</p>
      ...
      <Card className="glass-amber">
        <p className="flex items-start gap-2 text-[12px] font-medium text-ink-700"><ShieldAlert size={14} className="mt-0.5 shrink-0 text-amber-deep" /> Votre recommandation de fin de période d’essai est transmise à la RH/DRH pour décision formelle et envoi du courrier officiel. Vous ne prononcez pas la décision.</p>
      </Card>
